...
function setColour(hue) {
    ...
    if (typeof onlySetAccentColour === 'undefined') {
        setCssAccent('Dark', hslToHex(hue, 40, 5));
        setCssAccent('DarkIsh', hslToHex(hue, 50, 15));
        setCssAccent('LightIsh', hslToHex(hue, 50, 75));
...
function pickColour(hue) {
    ...
    if (
        // ensure new colour is more than 40deg from previous
        (Math.abs(parseInt(localStorage.getItem('last-colour')) - randomHue) <= 40)
        // ||
